...
def find_songs(url):
    try:
        ...
        albums = soup.find_all(
            "span",
            class_="!hidden md:!flex ptxt-album col-span-2 truncate text-ellipsis overflow-hidden pr-8",
        )
        durations = soup.find_all(
            "span", class_="inline-flex items-center col-span-1 align-self-end pl-6"
        )
    ...
    for i, track in enumerate(tracks):
        ...
        topics_a = soup.find_all(
            "a",
            class_="py-1 px-2 rounded border border-blue bg-transparent text-blue font-[500] transition-all ease-in-out duration-75 text-sm hover:bg-blue hover:text-white",
        )
        topics = []
        ...
        download_links = soup.find_all(
            "a",
            class_="inline-flex justify-center items-center js-download w-4 transition-colors ease-in-out duration-75 hover:text-blue",
        )
