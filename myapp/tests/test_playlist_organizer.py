...
class PlaylistOrganizerTest(TestCase):
    ...
    def test_reorganize_playlist(self):
        ...
        async def run_test():
            # Test adding a song
            result = await self.organizer.reorganize_playlist(
                "Add a song OMG at the beginning"
            )
            ...
            result = await self.organizer.reorganize_playlist(
                "remove Supernatural from the playlist"
            )
