...
class FavlistAPITestCase(TestCase):
    ...
    def test_update_favlist_as_owner(self):
        ...
        # Update without auth
        response = self.client.put(detail_url, {'name': 'New Name', 'songs': []}, format='json')
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_update_favlist_as_non_owner(self):
        ...
        response = self.client.put(detail_url, {'name': 'Hacked Name', 'songs': []}, format='json')
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_delete_favlist_as_non_owner(self):
        # Create favlist for user1
        favlist = Favlist.objects.create(name='Favlist Owned by User1', owner=self.user1)
        detail_url = f'/favlist/{favlist.id}/'
