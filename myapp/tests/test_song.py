...
class SongAPIPermissionTests(APITestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="regular_user", password="pass1234", is_superuser=False
        )

        # Create a superuser
        self.superuser = User.objects.create_superuser(
            username="admin_user", password="adminpass1234"
        )
...
class SongSearchViewTests(ESTestCase, APITestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="search_user", password="searchpass", is_superuser=False
        )
    ...
    def test_search_with_ai_param_true(self):
        ...
        self.assertEqual(response.data["count"], 1)
        self.assertIn(
            "AI Generated Track", [r["name"] for r in response.data["results"]]
        )
    ...
    def test_search_by_invalid_field(self):
        ...
        response = self.client.get(
            self.search_url + "?field=invalidfield&search=Pop&limit=20"
        )
